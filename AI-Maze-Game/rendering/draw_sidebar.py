...
def draw_sidebar(screen, screen_width, screen_height, selected_block_id, font_label, font_title, budget):
    ...
    for block_id in SELECTABLE_BLOCKS:
        ...
        is_locked = block_id == WALL_BLOCK
        cant_afford = budget < cost and not is_locked

        # dim the block if player cant afford it or it is display-only
        if cant_afford or is_locked:
        ...
        if block_id == selected_block_id and not is_locked:
            pygame.draw.rect(screen, (255, 255, 255), rect, 3)
        else:
            pygame.draw.rect(screen, border_color, rect, 2)

        # block name
        name_color = (100, 100, 100) if cant_afford or is_locked else (200, 200, 200)
        ...
        cost_text = "pre-placed" if is_locked else f"{cost} coins"
        cost_label_color = (80, 80, 80) if cant_afford or is_locked else (120, 120, 120)
